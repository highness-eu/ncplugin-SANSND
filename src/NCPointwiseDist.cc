...
NCP::PointwiseDist::PointwiseDist( VectD&& xvals, VectD&& yvals )
  : m_x(xvals), m_y(yvals)
{
...
std::pair<double,unsigned> NCP::PointwiseDist::percentileWithIndex(double p ) const
{
  nc_assert(p>=0.&&p<=1.0);
...
double NCP::PointwiseDist::commulIntegral( double x ) const
{
...
double NCP::PointwiseDist::sampleBelow( RNG& rng, double xtrunc ) const
{
  //Above or below edges is easy:
  if ( xtrunc <= m_x.front() ) {
    if ( xtrunc == m_x.front() )
...
double NCP::PointwiseDist::sampleBelowTrunc( RNG& rng, double xtrunc, double xmin ) const
{
